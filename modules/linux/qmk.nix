{
  ...
  opt = "withQmk";
  userGid = builtins.toString config.grazor.user.gid;
in {
  ...
  config = lib.mkIf cfg.${opt} {
    ...
    services.xserver.xkb.extraLayouts."enthium" = {
      description = "enthium";
      languages = ["ru"];
      symbolsFile = ./data/ru_eth;
    };

    services.udev.extraRules = ''
      ### Vial
      KERNEL=="hidraw*", SUBSYSTEM=="hidraw", ATTRS{serial}=="*vial:f64c2b3c*", MODE="0660", GROUP="${userGid}", TAG+="uaccess", TAG+="udev-acl"
