...
let
  ...
  xtensa = stdenv.mkDerivation rec {
    ...
    srcs = [
      (fetchurl {
        url =
          "https://github.com/esp-rs/rust-build/releases/download/v${version}/rust-${version}-x86_64-unknown-linux-gnu.tar.xz";
    ...
    buildInputs = [ openssl pkg-config zlib.out stdenv.cc.cc.lib ];
  ...
  xtensa_clang = stdenv.mkDerivation rec {
    ...
    buildInputs = [ python27 zlib.out stdenv.cc.cc.lib ];
